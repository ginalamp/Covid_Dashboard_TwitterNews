...
excess_deaths_provinces <- read_xlsx("data/weekly_excess_deaths_provinces.xlsx")

# weekly excess deaths per metro 
excess_deahts_metros <- read_xlsx("data/weekly_excess_deaths_metros.xlsx")
```
...
excess_deaths_provinces <- excess_deaths_provinces %>%
  mutate(date = ymd(date))

# look at the three time intervals
may3_june5 <- excess_deaths_provinces %>%
  filter(date <= ymd("2021-06-05")) %>%
  select(date, RSA)
may3_june5

may3_june12 <- excess_deaths_provinces %>%
  filter(date <= ymd("2021-06-12")) %>%
  select(date, RSA)
may3_june12

may3_june19 <- excess_deaths_provinces %>%
