...
ggplot(top_retweets_sentiment_per_outlet, aes(name, top_retweets_sentiment_mean, fill=name)) +
  geom_col() +
  labs(title = "Sentiment of Top 10 retweeted tweets per organisation on covid-19",
       x=NULL,
       y="Vader Sentiment Score") +
  scale_fill_manual(values = c(dailymav_colour, ewn_colour, news24_colour, sabc_colour)) +
  custom_column_basic_theme()
...
ggplot(top_favourited_sentiment_per_outlet, aes(name, top_favourited_sentiment_mean, fill=name)) +
  geom_col() +
  labs(title = "Sentiment of Top 10 favourited tweets per organisation on covid-19",
       x=NULL,
       y="Vader Sentiment Score") +
  scale_fill_manual(values = c(dailymav_colour, ewn_colour, news24_colour, sabc_colour)) +
  custom_column_basic_theme()
...
```{r}
tidy_tweets[[1]] <- tidy_tweets[[1]] %>% mutate(ymd(created_at))
tidy_tweets
```
...
ggplot(bind_rows(tidy_tweets, .id="df"), aes(created_at, rolling_avg, colour=screen_name)) + 
  ...
  labs(title= "Sentiment over time per media outlet",
       subtitle = paste("A rolling average of", NUM_TWEETS, "is used."),
       color="Media outlet",
       x = NULL,
       y = "Vader Sentiment Score") +
