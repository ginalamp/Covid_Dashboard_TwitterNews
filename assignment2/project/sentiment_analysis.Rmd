...
f <- function(suffix) {
  if (suffix == SA_SUFFIX) {
    ...
    sentiment_merged <- merge(overall_sentiment_per_outlet, top_retweets_sentiment_per_outlet, by = "name")
    sentiment_merged <- merge(sentiment_merged, top_favourited_sentiment_per_outlet, by="name")
    ...
    colnames(global_overall_sentiment_per_outlet)[1] <- "name"
    colnames(global_top_retweets_sentiment_per_outlet)[1] <- "name"
    colnames(global_top_favourited_sentiment_per_outlet)[1] <- "name"
    # condense df such that multiple series bar plot can be plotted
    sentiment_merged <- merge(global_overall_sentiment_per_outlet, global_top_retweets_sentiment_per_outlet, by = "name")
    sentiment_merged <- merge(sentiment_merged, global_top_favourited_sentiment_per_outlet, by="name")
    head(sentiment_merged)
    sentiment_melted <- melt(sentiment_merged %>% 
