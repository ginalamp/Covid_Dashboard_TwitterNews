...
top_words <- function(word_frequency_df, n_top_words) {
  ...
  top_n <- word_frequency_df %>%
    group_by(name) %>%
    slice_max(order_by = freq, n=n_top_words) %>%
    ungroup
  
  users <- unique(top_n[c("name")])
  ...
  for (i in seq(nrow(users[1]))) {
    top_n_of_org <- top_n %>% filter(name == users$name[i]) %>% select(word)
    top_n_per_org[[users$name[i]]] = head(top_n_of_org, n_top_words)
  }
...
extract_hashtags <- function(tidy_tweets_df) {
  return(tidy_tweets_df %>% filter(str_detect(word, "^#")))
}
...
# not enough hashtags used globally
...
ggplot() +
  geom_histogram(govZA_tweets %>% mutate(created_at = ymd_hms(created_at)), mapping = aes(x = created_at, fill = "All Tweets"), position = "identity", bins = 30, show.legend = TRUE) +
  geom_histogram(relevant_govZA, mapping = aes(x = created_at, fill = "Tweets related to Covid"), position = "identity", bins = 30, show.legend = TRUE) +
...
ggplot() +
  geom_histogram(all_tweets_SA %>% mutate(created_at = ymd_hms(created_at)) %>% filter(created_at > ymd_hms("2021-01-01 00:00:00")), mapping = aes(x = created_at, fill = "All Tweets"), position = "identity", bins = 30, show.legend = FALSE) +
  geom_histogram(tweets_SA, mapping = aes(x = created_at, fill = "Tweets related to Covid"), position = "identity", bins = 30, show.legend = FALSE) +
  scale_y_continuous(limits = c(0, 400)) +
  facet_wrap(~name, ncol = 1) +
  labs(title = "Frequency of Covid-related Tweets for various SA Media Outlets",
       x = "2021", y = "Tweet Count", color = "Tweets") +
    scale_color_manual(values = colors) +
  scale_fill_manual(values = ms_colors) +
  custom_column_basic_theme()
...
ggplot() +
  geom_histogram(all_tweets_global %>% mutate(created_at = ymd_hms(created_at)) %>% filter(created_at > ymd_hms("2021-01-01 00:00:00")), mapping = aes(x = created_at, fill = "All Tweets"), position = "identity", bins = 30, show.legend = FALSE) +
  geom_histogram(tweets_global, mapping = aes(x = created_at, fill = "Tweets related to Covid"), position = "identity", bins = 30, show.legend = FALSE) +
  scale_y_continuous(limits = c(0, 400)) +
  facet_wrap(~name, ncol = 1) +
  labs(title = "Frequency of Covid-related Tweets for various Global Media Outlets",
       x = "2021", y = "Tweet Count", color = "Tweets") +
    scale_color_manual(values = colors) +
  scale_fill_manual(values = c(ms_orange, ms_plum)) +
  custom_column_basic_theme()
```
...
```{r top hashtags per outlet}
# https://stackoverflow.com/questions/31796219/grid-table-and-tablegrob-in-gridextra-package
# https://cran.r-project.org/web/packages/gridExtra/vignettes/tableGrob.html#faster-tables-an-alternative-grid-function
grid.newpage()
grid.table(top_hashtags_SA)
```
