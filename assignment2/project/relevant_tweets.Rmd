...
tail(relevant_CapeTalk)

# filter for tweets related to covid
relevant_EWN <- EWN_tweets[grepl("covid", EWN_tweets[["text"]]) | 
                  grepl("Covid", EWN_tweets[["text"]]) |
                  grepl("corona", EWN_tweets[["text"]]) |
                  grepl("Corona", EWN_tweets[["text"]]) |
                  grepl("Pandemic", EWN_tweets[["text"]]) |
                  grepl("pandemic", EWN_tweets[["text"]]), ]
relevant_EWN <- relevant_EWN %>%
  mutate(created_at = ymd_hms(created_at))

tail(relevant_EWN)

# filter for tweets related to covid
relevant_News24 <- News24_tweets[grepl("covid", News24_tweets[["text"]]) | 
                  grepl("Covid", News24_tweets[["text"]]) |
                  grepl("corona", News24_tweets[["text"]]) |
                  grepl("Corona", News24_tweets[["text"]]) |
                  grepl("Pandemic", News24_tweets[["text"]]) |
                  grepl("pandemic", News24_tweets[["text"]]), ]
relevant_News24 <- relevant_News24 %>%
  mutate(created_at = ymd_hms(created_at))

tail(relevant_News24)
```
