...
# TODO
* Remove SA gov code (data read in and usage)
* (nice to have) update report graphs with new x-axis title size
...
SA_GOV_SUFFIX <- "SA Government" # TODO: remove
...
# TODO: update this to only have relevant fields
...
# SA government
govZA_tweets <- extract_relevant_data(read.csv("data/governmentza.csv")) # TODO: remove
...
tidy_tweets_govZA <- clean_tweets(relevant_govZA) # TODO: remove
```
...
plot_overall_topics(tidy_tweets_govZA, NUM_TOPICS_6, SA_GOV_SUFFIX) # TODO: remove
...
# TODO: added
topics_per_outlet_month_jan
topics_per_outlet_month_june
```
...
```{r get topic sentiment per media outlet: Jan and June}
# South Africa
...
```{r get mean sentiment per topic: Jan and June}
mean_sentiment_per_topic <- function(df) {
  sum_sentiment <- df %>%
...
# South Africa
topic_sentiment_per_outlet_month_jan <- lapply(vader_analysis_per_outlet_month_jan, mean_sentiment_per_topic)
topic_sentiment_per_outlet_month_june <- lapply(vader_analysis_per_outlet_month_june, mean_sentiment_per_topic)

# Global
global_topic_sentiment_per_outlet_month_jan <- lapply(global_vader_analysis_per_outlet_month_jan, mean_sentiment_per_topic)
global_topic_sentiment_per_outlet_month_june <- lapply(global_vader_analysis_per_outlet_month_june, mean_sentiment_per_topic)
```




```{r South Africa: get topic sentiment mean per month, outlet}
...
# vader_analysis_per_outlet_month_jan <- lapply(topics_per_outlet_month_jan, apply_vader)
# vader_analysis_per_outlet_month_june <- lapply(topics_per_outlet_month_june, apply_vader)
# sentiment_mean_per_outlet_month_jan <- unlist(lapply(vader_analysis_per_outlet_month_jan, get_sentiment_mean))
# sentiment_mean_per_outlet_month_june <- unlist(lapply(vader_analysis_per_outlet_month_june, get_sentiment_mean))
...
# overall_sentiment_per_outlet_month_jan <- data.frame(media_outlet=OUTLETS_TO_PLOT, sentiment_mean_per_outlet_month_jan)
# overall_sentiment_per_outlet_month_june <- data.frame(media_outlet=OUTLETS_TO_PLOT, sentiment_mean_per_outlet_month_june)
# head(overall_sentiment_per_outlet_month_jan)
# head(overall_sentiment_per_outlet_month_june)
```


```{r Global: get topic sentiment mean per month, outlet, and month per outlet}
# TODO: this needs changing - do not apply sentiment mean
# global_topics_per_outlet_month_jan
...
# global_vader_analysis_per_outlet_month_jan <- lapply(global_topics_per_outlet_month_jan, apply_vader)
# global_vader_analysis_per_outlet_month_june <- lapply(global_topics_per_outlet_month_june, apply_vader)
# global_sentiment_mean_per_outlet_month_jan <- unlist(lapply(global_vader_analysis_per_outlet_month_jan, get_sentiment_mean))
# global_sentiment_mean_per_outlet_month_june <- unlist(lapply(global_vader_analysis_per_outlet_month_june, get_sentiment_mean))
...
global_overall_sentiment_per_outlet <- data.frame(media_outlet=GLOBAL_OUTLETS_TO_PLOT, global_sentiment_mean_per_outlet)
head(global_overall_sentiment_per_outlet)

# global_overall_sentiment_per_outlet_month_jan <- data.frame(media_outlet=GLOBAL_OUTLETS_TO_PLOT, global_sentiment_mean_per_outlet_month_jan)
# global_overall_sentiment_per_outlet_month_june <- data.frame(media_outlet=GLOBAL_OUTLETS_TO_PLOT, global_sentiment_mean_per_outlet_month_june)
# head(global_overall_sentiment_per_outlet_month_jan)
# head(global_overall_sentiment_per_outlet_month_june)
```
...
<!-- 
# SA topics 2 and 5 (lockdown and president)
# Global topics 1 and 4 (lockdown and america president)
-->

```{r plot sentiment per media outlet per month}
...
```{r plot topics and month sentiment}
...
```{r plot sentiment per media outlet over time (overall, jan, june)}
plot_sentiment_topic_outlet_bar <- function(average, jan, june, suffix) {
    # set plot colors and labels
    plot_colors <- ms_colors
    legend_labels = c("Overall", "January", "June")
    ...
    sentiment_merged <- merge(average, jan, by = "media_outlet")
    sentiment_merged <- merge(sentiment_merged, june, by="media_outlet")
    if (suffix == SA_SUFFIX) {
        sentiment_melted <- melt(sentiment_merged %>%
                                   select(media_outlet, sentiment_mean_per_outlet, sentiment_mean_per_outlet_month_jan, sentiment_mean_per_outlet_month_june), id.vars=c("media_outlet"))
    } else {
        sentiment_melted <- melt(sentiment_merged %>%
                               select(media_outlet, global_sentiment_mean_per_outlet, global_sentiment_mean_per_outlet_month_jan, global_sentiment_mean_per_outlet_month_june), id.vars=c("media_outlet"))
    }

  # plot
  ggplot(sentiment_melted, aes(media_outlet, value, fill=variable)) +
    geom_bar(stat='Identity', position=position_dodge()) +
      labs(title = "Sentiment comparison over time per media outlet for their overall topics",
           subtitle = suffix,
           x=NULL,
           y="Vader Sentiment Score") +
...
plot_sentiment_topic_outlet_bar(overall_sentiment_per_outlet, overall_sentiment_per_outlet_month_jan, overall_sentiment_per_outlet_month_june, SA_SUFFIX)
plot_sentiment_topic_outlet_bar(global_overall_sentiment_per_outlet, global_overall_sentiment_per_outlet_month_jan, global_overall_sentiment_per_outlet_month_june, GLOBAL_SUFFIX)
```
