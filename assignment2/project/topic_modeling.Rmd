...
vader_analysis_per_outlet_month_jan
```
```{r}
...
topic_sentiment_per_outlet_month_jan
```
...
```{r}
x <- bind_rows(topic_sentiment_per_outlet_month_jan, .id = "id")
x
```
```{r}
ggplot() +
  geom_col(x, mapping = aes(topic, compound, fill=topic), stat='Identity', position=position_dodge()) +
...
```{r}
y <- melt(x %>%
            select(outlet, compound, topic), id.vars=c("outlet"))
y
```
...
```{r}
ggplot(y, aes(outlet, value, fill=variable)) +
