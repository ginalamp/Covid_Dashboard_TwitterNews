...
```{r add months as an additional column}
...
```{r}
v <- lapply(topics_per_month_list, apply_vader_month)
```


```{r}
montly_topic_sentiment <- lapply(v, get_mean_sentiment_per_month)
montly_topic_sentiment
```
...
plot_topic_month <- function(df, suffix) {
  ...
  fill = format(df$month, "%B")
  
  # plot
  ggplot() +
    geom_col(bind_rows(df, .id = "id"), mapping = aes(month, compound, fill=factor(topic)), stat='Identity', position=position_dodge()) +
...
plot_topic_month(montly_topic_sentiment, SA_SUFFIX)
```
