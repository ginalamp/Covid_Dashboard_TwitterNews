...
extract_relevant_data <- function(df) {
  return(df %>% select(status_id, screen_name, created_at, statuses_count, is_retweet, favourites_count, retweet_count, followers_count, friends_count))
}
```
...
```{r get number of followers, friends, tweets, retweets per media outlet}
...
# calculate media organisation's number of retweets (is_retweet == TRUE)
org_retweet_count <- unlist(lapply(media_outlet_list, function(df) {return(NROW(df$status_id[df$is_retweet]))}))
...
number_of_x <- data.frame(name=names, 
                          ...
                          org_retweet_count=org_retweet_count)
head(number_of_x)
```
```{r}
# select tweet IDs that are retweets
a <- CapeTalk_tweets$status_id[(CapeTalk_tweets$is_retweet)]
# dim(a)
NROW(a)
```
