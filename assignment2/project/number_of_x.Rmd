...
plot_numeric_bar <- function(df, col_plot, title, title_suffix, covid_note, set_scale) {
  # set plot colors
  ...
  suffix = THOUSAND
  scale = 1e-3
  if (set_scale == MILLION) {
    suffix = MILLION
    scale = 1e-6
  }
  
  # plot
  ggplot(df, aes(name, col_plot, fill=name)) +
    geom_col() +
    labs(title = paste("Number of", title, "per media outlet", covid_note),
...
```{r plot South African numerical analysis}
plot_numeric_bar(number_of_x, number_of_x$followers_count, "followers", SA_SUFFIX, NULL, MILLION)
plot_numeric_bar(number_of_x, number_of_x$tweets_count, "tweets", SA_SUFFIX, NULL, THOUSAND)
plot_numeric_bar(number_of_x, number_of_x$user_retweet_sum, "retweets", SA_SUFFIX, NULL, THOUSAND)

plot_numeric_bar(covid_number_of_x, covid_number_of_x$user_retweet_sum, "retweets", SA_SUFFIX, COVID, THOUSAND)
```
```{r plot Global numerical analysis}
plot_numeric_bar(global_number_of_x, global_number_of_x$followers_count, "followers", GLOBAL_SUFFIX, NULL, MILLION)
plot_numeric_bar(global_number_of_x, global_number_of_x$tweets_count, "tweets", GLOBAL_SUFFIX, NULL, THOUSAND)
plot_numeric_bar(global_number_of_x, global_number_of_x$user_retweet_sum, "retweets", GLOBAL_SUFFIX, NULL, MILLION)

plot_numeric_bar(covid_global_number_of_x, covid_global_number_of_x$user_retweet_sum, "retweets", GLOBAL_SUFFIX, COVID, THOUSAND)
```
...
<!----------------------------------------------------------------------------------------------------------------------------------------------------->
<!----------------------------------------------------------------------------------------------------------------------------------------------------->
<!-- TODO:  @JOHANNA below here are the individual graphs (above I condensed it) - could you please have a look to see whether we can group the graphs? -->
<!----------------------------------------------------------------------------------------------------------------------------------------------------->

```{r}
# https://stackoverflow.com/questions/29131976/multiple-series-barplot
library(reshape2)
number_of_x_melted = melt(number_of_x, id.vars=c("name"))
